...
namespace Tagtag
{
    public static class CollectionBook
    {
        ...
        public static bool FreshLocation(LocationFix location, long now)
        {
            return location != null && !double.IsNaN(location.latitude) && !double.IsInfinity(location.latitude)
                ...
                && !float.IsNaN(location.accuracyMeters) && location.accuracyMeters >= 0 && location.accuracyMeters <= 50
