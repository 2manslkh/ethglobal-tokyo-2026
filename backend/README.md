...
Explore accepts fresh location fixes up to 5,000 metres accuracy for `/v1/nearby`, with its 2 km search radius unchanged. AR recovery and collection accept accuracy up to 5,000 metres and permit distance to the sticker up to measured accuracy plus 100 metres. Collection still requires a valid five-minute discovery and tracked AR sticker tap in the client. Automatic publication retains 100-metre accuracy. All fixes retain the 30-second freshness limit. The collection response includes `isNew` so the client can show a first-collection celebration without a separate collection-list request. Deploy the backend before distributing the updated client.
...
Publication accepts the exact built-in IDs `taggi-1` through `taggi-13` and preserves each ID in publication and collection responses. The existing souvenir contract has four variants: `taggi-1` through `taggi-4` map to NFT presets 0 through 3 respectively. `taggi-5` through `taggi-13` and custom designs mint the generic NFT preset 0. No new contract metadata variant is required for these added app presets.
