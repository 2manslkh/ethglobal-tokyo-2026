...
`validate` and `plan` are local only. `preflight` makes read-only project, billing, Firebase, and relevant worker checks. It stops if billing is disabled. The deploy path requires an explicit confirmation and is reserved for the coordinator after the blocker is resolved:
