...
[PROJECT NAME] is an AR app built in Unity to allow users to place and find stickers in the real world. These stickers are collectible NFTs that represent unique locations for individuals.
...
opens a discovery starter screen with a working button. AR placement, locations,
...
/Applications/Unity/Hub/Editor/6000.5.5f1/Unity.app/Contents/MacOS/Unity \
  ...
  -executeMethod BuildIos.Build -logFile /tmp/stickerhunt-unity.log
```

The scene is the build entry point, and the development bundle ID is
`com.kenk.stickerhunt`. Unity writes the Xcode project to `Build/iOS`, which is
...
xcrun devicectl device install app --device YOUR_DEVICE_ID \
  Build/DerivedData/Build/Products/Debug-iphoneos/StickerHunt.app
xcrun devicectl device process launch --device YOUR_DEVICE_ID com.kenk.stickerhunt
```
