...
Separate ARKit preparation and final iOS export succeeded. Both unsigned and development-signed Xcode builds passed. The new Core Location bridge is in the Sources phase and linked binary. Exported and built plist metadata enable visible dark native status text. No iPhone was detected over USB during final checks, so installation and real publishing latency remain unverified.
