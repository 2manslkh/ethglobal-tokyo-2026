...
Thirdweb Unity SDK 6.1.3 creates an embedded EOA using the existing Firebase ID token. Enable custom JWT authentication in the Thirdweb project for issuer `https://securetoken.google.com/tagtag-tokyo-2026`, audience `tagtag-tokyo-2026`, and Firebase's signing keys. Restrict the public client ID to the app's bundle ID, `com.kenk.tagtag`, using the provider's application settings. Verify the same Firebase UID restores the same address on two devices before enabling rollout.
...
On-chain token IDs are random. Metadata includes only generic Taggi art and descriptions. Firebase IDs, authored text, notes, world maps, coordinates, and discovery timestamps are not NFT metadata. Transaction timing and recipient wallet addresses are public. App-approved discovery remains a gameplay check, not cryptographic proof of presence. Transferring a token does not grant note access or change the original collector's book.
...
1. Run the tests below and configure Thirdweb custom JWT authentication. Do not enable paid provider services automatically.
2. Pin the four generic JSON artifacts under `contracts/metadata/` to IPFS. Each includes its preset PNG as a data URI. Record their immutable `ipfs://` URIs; no user content is pinned.
...
4. Configure the HTTP API with `NFT_ENABLED=true`, `NFT_CHAIN_ID=11155111`, `NFT_CONTRACT_ADDRESS`, and `NFT_WALLET_DOMAIN=tagtag-api-329004805254.asia-northeast1.run.app`. Deploy the Firestore indexes. The HTTP API receives no signer private key.
5. Run a separate Cloud Run job from the backend image with command `node src/mint-worker.js`. In addition to the API's NFT configuration, provide `NFT_RPC_URL` and inject `NFT_SIGNER_PRIVATE_KEY` from Secret Manager. Give only this worker identity access to that secret. Use the existing Firebase project and bucket settings required by the adapter. Configure one task and one parallel execution, and schedule it every minute. Application leases also protect overlapping executions.
...
Live minting is not verified until Thirdweb configuration, IPFS pinning, a funded signer, a deployed contract, and device checks are recorded in [device verification](DEVICE_VERIFICATION.md). Local mocks and an unsigned Xcode build do not establish that live flow.
