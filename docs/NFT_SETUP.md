...
1. Follow [isolated staging setup](NFT_STAGING.md), run the tests below and configure Thirdweb custom JWT authentication. Do not enable paid provider services automatically.
...
4. Configure the staging HTTP API with `NFT_ENABLED=true`, `NFT_CHAIN_ID=11155111`, `NFT_CONTRACT_ADDRESS`, and `NFT_WALLET_DOMAIN=tagtag-api-542095619867.asia-northeast1.run.app`. Deploy the staging Firestore indexes. The HTTP API receives no signer private key. Production activation requires a separate rollout.
5. Run a separate Cloud Run job from the backend image with command `node src/mint-worker.js`. In addition to the API's NFT configuration, provide `NFT_RPC_URL` and inject `NFT_SIGNER_PRIVATE_KEY` from Secret Manager. Give only this worker identity access to that secret. Use the staging Firebase project and bucket settings required by the adapter. Configure one task and one parallel execution, and schedule it every minute. Application leases also protect overlapping executions.
