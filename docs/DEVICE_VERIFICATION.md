...
## Live Sepolia mint and profile link — 2026-09-27

On Dawg., the signed-in collector completed a new in-app discovery, “I was here at ethglobal”. The app’s collected-sticker sheet displayed “Souvenir NFT pending”, and Firestore contained one `nftMints` record addressed to the phone wallet `0xBC5fc5e8EBd5611DdE4b56C88236F5878E85bACb`. Cloud Run execution `tagtag-nft-mint-ncpd8` exited successfully and submitted transaction `0xd13cee7be67abf116654c1f92afeb208432a6ad6fe3f381ae4443644895f6b05`. Its Sepolia receipt succeeded (`status=1`) and emitted ERC-721 `Transfer` from zero to that wallet for token `36556125107457144972077680615596633412849388362148149099364433991239160613193`. `ownerOf` returned the same wallet and `tokenURI` returned `https://tagtag-tokyo-2026.web.app/nft/v1/taggi-1.json`. After Sepolia finalized block 11787802, Cloud Run execution `tagtag-nft-mint-g4wkv` marked the Firestore mint `confirmed` with one signing attempt. A later CoreDevice capture of the still-open collected-sticker sheet showed “Souvenir NFT minted on Sepolia.” The capture stays outside the repository because it includes the collector's private note.

The profile now renders the full ready-wallet address as a tappable button that opens its Sepolia Etherscan page. The visible profile test failed before this change and passed after it, including pending → ready visibility; full Edit Mode passed **259/259**. Separate ARKit preparation, Unity iOS export, and signed Xcode Debug/iphoneos build succeeded. The updated app was installed over the existing build on Dawg. and launched without losing its collected sticker. A profile-button tap on the physical device has not yet been observed.

Cloud Scheduler job `tagtag-nft-mint-every-5m` is enabled in `asia-northeast1` with a dedicated invoker identity and five-minute cadence. An explicit Scheduler dispatch started Cloud Run execution `tagtag-nft-mint-dp8gt`, which completed successfully. The job's signer secret remains accessible only to the mint worker identity. This setup replaces manual-only queue processing for later collections.

The first post-mining reconciliation marked the record `delayed` with `reconciliation_failed`; its broad catch did not retain the underlying exception. Code inspection found that a mined but unfinalized receipt entered the rebroadcast path. The worker now keeps a canonical, mined receipt `submitted` until finality instead of rebroadcasting it. The regression test failed before the fix and passed after it; the complete backend suite passed **85/85** with one emulator-only skip. Worker image `sha256:83ba752f5bf0d764d2af0df07f0e649a420edd3257aae53d9a9257e046685130` is deployed only to the mint job. It confirmed the same transaction without another signing attempt.
